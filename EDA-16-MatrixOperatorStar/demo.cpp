...
// Fx4(&j);    // No se imprime, ya que q es nullptr

void DemoSmartPointers() {
    CArray<TraitArrayIntInt> v2("Lucero");
    CArray<TraitArrayIntInt> *pX; //, *pV3 = new CArray("Luis");

    shared_ptr<CArray<TraitArrayFloatString>> pV3(new CArray<TraitArrayFloatString>("Luis"));
    shared_ptr<CArray<TraitArrayFloatString>> pV4;
    pV4 = pV3;
    auto &rA = *pV3;
...
void DemoArray() {
    ...
    for (auto i = 0; i < 15; i++)
        v1.insert(i, i + 5); // Inserta un elemento en v1 en la posición i con el valor i+5
    ...
    auto& rA = *pV3;
    for (auto i = 100; i < 112; i++) {
        v2.insert(sqrt(i), i); // Inserta un elemento en v2 con la raíz cuadrada de i y el valor i
        pV3->insert(i, sqrt(i)); // Inserta un elemento en pV3 con el valor i y la raíz cuadrada de i
        // (*pv3).insert(i);
        // rA.insert(i);
    }
    cout << "Printing V1 (TraitArrayIntInt)" << endl;
    cout << v1; // Imprime el contenido de v1 (usando el operador sobrecargado <<)
    ...
    pV3->print(cout);
    // (*pV3).print();     *pV3 ya es un objeto
    // rA.print();          rA también es un objeto
    // pV3[0].print();      pV3 también es un array con solo 1 elemento [0]
    // (*(pV3+0)).print();
    // (*(0+pV3)).print();
    // 0[pV3].print();
    // delete pV3;
