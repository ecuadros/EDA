...
struct tagObjectInfo
{
        ...
        V value;
        size_t UseCounter;
        tagObjectInfo(const T &_key, V _value): key(_key), value(_value), UseCounter(0) {}
        tagObjectInfo(const tagObjectInfo &objInfo): key(objInfo.key), value(objInfo.value), UseCounter(0) {}
        tagObjectInfo() {}
...
template <typename T, typename V>
CBTreePage<T, V>::CBTreePage(size_t maxKeys, bool unique): m_MaxKeys(maxKeys), m_Unique(unique), m_KeyCount(0)
{
       Create();
...
template <typename T, typename V>
template <typename CompareFn>
bt_ErrorCode CBTreePage<T, V>::Insert(const T& key, const V value, CompareFn compareFn)
{
       size_t pos = binary_search(m_Keys, 0, m_KeyCount, key, compareFn);
       bt_ErrorCode error = bt_ok;

       if( pos < m_KeyCount && (T)m_Keys[pos] == key && m_Unique)
...
template <typename T, typename V>
bool CBTreePage<T, V>::Redistribute1(size_t &pos)
{
...
template <typename T, typename V>
bool CBTreePage<T, V>::Redistribute2(size_t pos)
{
...
template <typename T, typename V>
void CBTreePage<T, V>::RedistributeR2L(size_t pos)  
{
...
template <typename T, typename V>
void CBTreePage<T, V>::RedistributeL2R(size_t pos)
{
       BTPage  *pSource = m_SubPages[pos],
                       *pTarget = m_SubPages[pos+1];
       while(pSource->GetNumberOfKeys() > pSource->MinNumberOfKeys() &&
...
template <typename T, typename V>
void CBTreePage<T, V>::SplitChild(size_t pos)
{
...
template <typename T, typename V>
void CBTreePage<T, V>::SplitPageInto3(vector<ObjectInfo>& tmpKeys, vector<BTPage *>  & tmpSubPages, BTPage* & pChild1, BTPage* & pChild2, BTPage* & pChild3, ObjectInfo & oi1, ObjectInfo & oi2)
{
...
template <typename T, typename V>
template <typename CompareFn>
bool CBTreePage<T, V>::Search(const T &key, V &value, CompareFn compareFn)
{
       size_t pos = binary_search(m_Keys, 0, m_KeyCount, key, compareFn);
       if( pos >= m_KeyCount )
       {    if( m_SubPages[pos] )
...
template <typename T, typename V>
template <typename Callable, typename Iter, typename... Args>
void CBTreePage<T, V>::ForEachReverse(Callable fn, Iter level, Args&&... args)
{
...
template <typename T, typename V>
template <typename Callable, typename Iter, typename... Args>
void CBTreePage<T, V>::ForEach(Callable fn, Iter level, Args&&... args)
{
...
template <typename T, typename V>
template <typename Callable, typename Iter, typename... Args>
typename CBTreePage<T, V>::ObjectInfo *
CBTreePage<T, V>::FirstThat(Callable fn, Iter level, Args&&... args)
{
        ObjectInfo *pTmp;
        for (size_t i = 0; i < m_KeyCount; i++)
        {
...
template <typename T, typename V>
template <typename CompareFn>
bt_ErrorCode CBTreePage<T, V>::Remove(const T &key, const V value, CompareFn compareFn)
{
       bt_ErrorCode error = bt_ok;
       size_t pos = binary_search(m_Keys, 0, m_KeyCount, key, compareFn);
...
template <typename T, typename V>
bt_ErrorCode CBTreePage<T, V>::Merge(size_t pos)
{
...
template <typename T, typename V>
bt_ErrorCode CBTreePage<T, V>::MergeRoot()
{
       size_t pos = 1;
       assert( m_SubPages[pos-1]->NumberOfKeys() +
                       m_SubPages[ pos ]->NumberOfKeys() +
...
template <typename T, typename V>
typename CBTreePage<T, V>::ObjectInfo &
CBTreePage<T, V>::GetFirstObjectInfo()
{
        if( m_SubPages[0] )
...
template <typename T, typename V>
void Print(tagObjectInfo<T, V> &info, size_t level, void *pExtra)
{
...
template <typename T, typename V>
void CBTreePage<T, V>::Print(ostream & os)
{
       ForEach(&::Print<T, V>, 0, &os);
}

template <typename T, typename V>
void CBTreePage<T, V>::Create()
{
       Reset();
...
template <typename T, typename V>
void CBTreePage<T, V>::clear()
{
       m_KeyCount = 0;
}

template <typename T, typename V>
CBTreePage<T, V> * CreateBTreeNode (size_t maxKeys, bool unique)
{
       return new CBTreePage<T, V> (maxKeys, unique);
}

template <typename T, typename V>
void CBTreePage<T, V>::MovePage(BTPage *pChildPage, vector<ObjectInfo> &tmpKeys, vector<BTPage *> &tmpSubPages)
{
