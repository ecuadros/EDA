...
//    using value_type      = typename Traits::value_type;
//    using LinkedValueType = typename Traits::LinkedValueType;
//    using Base = NodeBinaryTree<Traits>;
//   private:
...
//   public:
//     NodeAVL(Node *pParent,value_type elem,LinkedValueType elem2, Node *p0 = nullptr, Node *p1 = nullptr) 
//         :Base(pParent,elem,elem2,p0,p1),m_depth(0){
//           m_depth+=10;
//           //this->m_pParent = pParent; 
//           this->m_pParent = static_cast<Node*>(pParent);
//         }
//     value_type getData(){
//       return Base::getData();  
//     }  
...
public: 
    ...
    void Update_AVL_Tree(Node *pNode){
      ...
      while(pNode->getParent()){
        ...
        if(balanceFactor>1){
           pNode=general_rotation_(pNode,pNode_tmp,1);
        }
        else{
          if(balanceFactor<-1){
            pNode=general_rotation_(pNode,pNode_tmp,-1);
  ...
  Node* general_rotation_(Node *pNode,Node *&pNode_tmp,int dir){
      int dir_x,dir_y=0;
      if(dir==1){
        dir_x=1;
        dir_y=0;
      ...
      if(!pParent->getParent()){
          ...
          Node *pGParent=pNode->getParent()->getParent();
          Node *pParent=pNode->getParent();
          //>>>
          bool c1=   pGParent->getData()>pParent->getData()&& pParent->getData()>pNode->getData()&&pNode->getData()>pNode_tmp->getData();
          //<<<
          bool c1_e= pGParent->getData()<=pParent->getData()&& pParent->getData()<=pNode->getData()&&pNode->getData()<=pNode_tmp->getData();
          //>><
          bool c2=   pGParent->getData()>pParent->getData()&& pParent->getData()>pNode->getData()&&pNode->getData()<=pNode_tmp->getData();
          //<<>
          bool c2_e= pGParent->getData()<=pParent->getData()&& pParent->getData()<=pNode->getData()&&pNode->getData()>pNode_tmp->getData();
          //><<
          bool c3=   pGParent->getData()>pParent->getData()&& pParent->getData()<=pNode->getData()&&pNode->getData()<=pNode_tmp->getData();
          //<>>
          bool c3_e= pGParent->getData()<=pParent->getData()&& pParent->getData()>pNode->getData()&&pNode->getData()>pNode_tmp->getData();
          //><>
          bool c4=   pGParent->getData()>pParent->getData()&& pParent->getData()<=pNode->getData()&&pNode->getData()>pNode_tmp->getData();
          //<><
          bool c4_e= pGParent->getData()<=pParent->getData()&& pParent->getData()>pNode->getData()&&pNode->getData()<=pNode_tmp->getData();
          //Case 1 
          if(c1||c1_e){
              if(!pNode->getChild(dir_x)){
          ...
          if(c2||c2_e){
                ...
                if(!pChild1_z){
                  ...
                  pNode->setpChild_nc(pChild1_z,dir_x);//ok
                  pChild1->setParent(pGParent);
                  pNode->setParent(pChild1);
                  pParent->setParent(pChild1);
                  pParent->setpChild_nc(nullptr,dir_y);
                  pChild1_z->setParent(pNode);
          ...
          if(c4||c4_e){
              ...
              // }
              // else{
              //   pNode->getChild(dir_x)->setpChild_nc(pParent,dir_x);
              //   pParent->setParent(pNode->getChild(dir_x));
              //   pParent->setp_height(1+max(height(pParent->getChild(0)), height(pParent->getChild(1))));
              //   pNode->getChild(dir_x)->setp_height(1+max(height(pNode->getChild(dir_x)->getChild(0)), height(pNode->getChild(dir_x)->getChild(1))));
              // }
              // pParent->setpChild_nc(nullptr,dir_y);
              // Node *pTemp=pNode;
              // while(pTemp){
