...
void DemoDynamicMatrixes()
{
    ...
    cout << "----------------mat1*mat2--------------------" << endl;
    // DONE: overload operator*(CMatrix<Traits> &other)
    CMatrix<MatrixTraitFloat> mat3 = mat1 * mat2;
    cout << mat3;

    cout << endl;
    cout << "----------------Iterator matrix--------------------" << endl;
    // DONE: Create Iterator for CMatrix
    cout << endl << "----mat1----" << endl;
    cout<< "All 12 elements: ";
    ...
    // // DONE: prepare Matrix to be used as a matrix from outside
...
void DemoBTree()
{
    ...
    if(val) {
        cout<< "First that is 10: " << val->key << " -> " << val->ObjID << endl;
    } else {
    ...
    cout<< endl;
    // foreach(bt.m_Root.begin(), bt.m_Root.end(), [](auto &n){ cout << ""; });
    // bt.ForEach([](auto &n){ cout << n << " "; }, 0, nullptr);
}
...
void DemoLinkedList()
{
    ...
    // // Funcion foreach
    // auto fx = [](auto &n){ n->m_data *= 2; };
    // foreach(list, fx);  
    
    // // Operador << y mostrar resultado de foreach
    // cout << "Foreach (multiplicar por 2): " << list;
